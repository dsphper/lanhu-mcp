...
**数据源说明：**
- **sketch.json（主要数据源）**：包含完整的图层树结构、样式属性、文本内容、切图引用。这是本技能的核心解析对象。
- **schema.json（可选补充）**：蓝湖 DDS 高级功能，包含组件化和交互信息。当前版本中此文件通常为空，本技能暂时忽略此文件，未来可扩展支持。
...
**重要说明：** `--framework` 参数**仅影响代码示例的语法**，不影响 Markdown 文档结构。

所有框架都输出相同的 Markdown 文档结构（层级树、区域示意图、Flex 属性、间距表格），差异仅体现在：
- **Flex 属性提示** 中的代码示例语法
- **实现建议** 中的组件命名和结构建议
- **components.md** 中的组件描述语法
