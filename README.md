...
- **原始切图自动交付（新增）**：实际下载并校验图片尺寸/哈希，提供可通过 MCP 传输的资源包和客户端安装器。详见 [设计上下文与资源交付](DESIGN_CONTEXT.md)。
...
| `lanhu_export_design_assets` | 下载校验原始素材并提供资源包 | 自动安装到客户端工程 |
...
| `lanhu_get_members` | 查看MCP访问记录中的协作者 | 本地协作记录 |
